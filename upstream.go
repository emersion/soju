...
func (uc *upstreamConn) handleMessage(msg *irc.Message) error {
	...
	case "MODE":
		...
		if !uc.isChannel(name) { // user mode change
			...
			if ch.modes != nil {
				if err := ApplyChannelModes(uc.availableChannelModes, ch.modes, modeStr, msg.Params[2:]...); err != nil {
					return err
				}
			}
	...
	case irc.RPL_CHANNELMODEIS:
		...
		if err := ApplyChannelModes(uc.availableChannelModes, ch.modes, modeStr, msg.Params[3:]...); err != nil {
			return err
		}
		if firstMode {
			modeStr, modeParams := ChannelModeString(ch.modes)
